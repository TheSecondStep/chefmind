...
4. 使用说明
-使用前请先启动MCP服务器，见其他仓库， MCP服务器路径为硬编码，请根据实际情况修改。

- 输入查询内容，Agent 会根据用户偏好推荐食谱
- 输入 `exit` 退出程序
  
- 输入"@解析食材"，解析本地路径下的食材|购物小票图片，识别食材名称、生产日期、保质期。
- 输入"@本地食谱"，根据用户输入和当前食材，查找data/recipes目录下的食谱文件，推荐食谱。
- 输入"@解析小红书食谱"，根据小红书链接，解析食谱, 并根据菜名作为文件名，保存到data/recipes目录下， 如果文件已存在，则更新文件。
- 输入"@CookLikeHOC"，根据用户输入和当前食材，推荐相关的老乡鸡食谱。
- 输入"@下厨房食谱"，根据用户输入和当前食材，搜索下厨房食谱数据集并推荐。
